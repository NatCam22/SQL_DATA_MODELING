...
create table academia.vehiculo(
...
create table academia.importe(
id serial primary key,
valor float not null,
moneda varchar(20) not null
);
create table academia.revision(
id serial primary key,
kilometros int not null,
fecha date not null,
id_importe int not null,
matricula varchar(10) not null,
constraint revision_vehiculo_fk foreign key(matricula) references vehiculo(matricula),
constraint importe_revision_fk foreign key(id_importe) references importe(id)
);

create table academia.marca(
nombre varchar(10) primary key,
grupo_empresarial varchar(40) not null
);

alter table academia.vehiculo add constraint marca_coche_fk foreign key(nombre_marca) references marca(nombre);

create table academia.seguro(
id serial primary key,
numero_poliza int not null,
aseguradora varchar(10) not null,
fecha_fin date not null,
matricula varchar(10) not null,
constraint seguro_vehiculo_fk foreign key(matricula) references vehiculo(matricula)
);
...
insert into marca (nombre, grupo_empresarial) select distinct marca, grupo grupo_empresarial from coches;

insert into importe (valor, moneda) select distinct importe_revision, moneda from coches;

insert into vehiculo(matricula, nombre_marca, modelo, fecha_compra, color, kilometraje) select distinct matricula, marca, modelo, fecha_compra, color, kms_totales from coches;

insert into seguro (numero_poliza, aseguradora, fecha_fin, matricula) select distinct n_poliza, aseguradora, fecha_alta_seguro, matricula from coches;

insert into revision (kilometros, fecha, id_importe, matricula) select kms_revision, fecha_revision, id id_importe, matricula from coches
inner join importe 
on importe_revision = importe.valor 
and coches.moneda = importe.moneda;
...
from vehiculo v
left join seguro s 
on s.matricula = v.matricula 
left join marca m 
on m.nombre = v.nombre_marca 
