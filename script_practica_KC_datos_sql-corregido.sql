...
constraint vehiculo_color_fk foreign key (id_color) references flota.color(id)
);

create table flota.divisa(
id serial primary key,
nombre varchar(60) not null
);
...
constraint revision_vehiculo_fk foreign key(matricula) references flota.vehiculo(matricula),
constraint revision_divisa_fk foreign key(id_divisa) references flota.divisa(id)
);
...
constraint marca_grupo_empresarial_fk foreign key(id_grupo_empresarial) references flota.grupo_empresarial(id)
);
...
constraint modelo_marca_fk foreign key(id_marca) references flota.marca(id)
);
alter table flota.vehiculo add constraint modelo_vehiculo_fk foreign key(id_modelo) references flota.modelo(id);
...
matricula varchar(10) not null,
constraint seguro_aseguradora_fk foreign key(id_aseguradora) references flota.aseguradora(id),
constraint seguro_vehiculo_fk foreign key(matricula) references flota.vehiculo(matricula)
);
...
--Tablas sin FK
insert into flota.color(nombre) select distinct color from coches;

insert into flota.grupo_empresarial(nombre) select distinct grupo from coches;

insert into flota.aseguradora(nombre) select distinct aseguradora from coches;

insert into flota.divisa(nombre) select distinct moneda from coches;

--Tablas que tienen un FK con referencia a las anteriores.

insert into flota.marca(nombre, id_grupo_empresarial) select c.marca, ge.id from coches c inner join flota.grupo_empresarial ge on c.grupo = ge.nombre group by 1,2;

insert into flota.modelo(nombre, id_marca) select c.modelo, m.id from coches c inner join flota.marca m on c.marca = m.nombre group by 1,2;

--Tablas con 2 FK

insert into flota.vehiculo(matricula, id_modelo, kilometraje, id_color, fecha_compra) 
select distinct c.matricula, m.id, c.kms_totales, col.id, c.fecha_compra 
from coches c inner join flota.modelo m on c.modelo = m.nombre inner join flota.color col on c.color = col.nombre;

insert into flota.seguro(numero_poliza, id_aseguradora, fecha_fin, matricula) 
select c.n_poliza, a.id, c.fecha_alta_seguro, v.matricula 
from coches c inner join flota.aseguradora a on c.aseguradora  = a.nombre 
inner join flota.vehiculo v on c.matricula = v.matricula
group by 1,2,3,4;

insert into flota.revision (kilometros, fecha, id_divisa, matricula, valor) 
select distinct c.kms_revision, c.fecha_revision, d.id, v.matricula, c.importe_revision 
from coches c inner join flota.divisa d on d.nombre = c.moneda 
inner join flota.vehiculo v on c.matricula = v.matricula;
...
grupo_empresarial.nombre as grupo_empresarial,
...
from flota.vehiculo
left join flota.seguro 
on seguro.matricula = vehiculo.matricula
left join flota.modelo
on vehiculo.id_modelo = modelo.id 
left join flota.marca 
on marca.id  = modelo.id_marca 
left join flota.grupo_empresarial
on marca.id_grupo_empresarial = grupo_empresarial.id
left join flota.color
on vehiculo.id_color = color.id 
left join flota.aseguradora
on seguro.id_aseguradora = aseguradora.id 
---Nota, por construcción de las tablas y del query los datos que se obtienen usando left o inner join son equivalentes en los resultados del query.
